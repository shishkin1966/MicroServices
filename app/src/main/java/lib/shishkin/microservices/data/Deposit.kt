...
@Entity(tableName = Deposit.TABLE)
class Deposit() : Parcelable {
    companion object CREATOR : Parcelable.Creator<Deposit> {

        override fun createFromParcel(parcel: Parcel): Deposit {
            return Deposit(parcel)
        }

        override fun newArray(size: Int): Array<Deposit?> {
            return arrayOfNulls(size)
        }

        const val TABLE = "Deposit"
